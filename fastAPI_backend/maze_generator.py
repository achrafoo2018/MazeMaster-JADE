...
class MazeGenerator:
	...
	def random_maze_generator(self, r, c, P0, Pf):
		ROWS, COLS = r, c
		...
		while S.not_empty():
			...
			if (x + 1 < ROWS) and maze[x + 1][y] == 0 \
			and previous[x][y] != (x + 1, y):
				continue
			if (0 < x) and maze[x-1][y] == 0 \
			and previous[x][y] != (x-1, y):
				continue
			if (y + 1 < COLS) and maze[x][y + 1] == 0 \
	...
	def generate_and_save_maze(self, rows, cols, start, end, filename):
		# Generate the maze
		maze = self.random_maze_generator(rows, cols, start, end)
